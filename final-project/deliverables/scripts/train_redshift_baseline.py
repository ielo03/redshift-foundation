...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Initialize model weights from a saved state_dict without resuming optimizer/epoch/best metrics.",
    )
    parser.add_argument(
        "--save-epoch-checkpoints",
        action="store_true",
        help="Also save numbered per-epoch checkpoints. By default only checkpoints/latest.pt is overwritten.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.input.suffix == ".pt":
        ...
        X = batch["flux"].numpy()
        if args.use_ivar_channel and "ivar" in batch:
            ivar = batch["ivar"].numpy()
        elif args.use_ivar_channel:
            ivar = np.ones_like(X, dtype=np.float32)
        else:
            ivar = None
        if args.use_validity_channel and "mask" in batch:
            observed_valid = (~batch["mask"]).numpy().astype(np.float32)
        elif args.use_validity_channel:
            observed_valid = np.ones_like(X, dtype=np.float32)
        else:
            observed_valid = None
        ...
        X = batch["flux"].numpy()
        ivar = batch["ivar"].numpy() if args.use_ivar_channel else None
        observed_valid = (~batch["mask"]).numpy().astype(np.float32) if args.use_validity_channel else None
    ...
    n_records = len(X)
    shared_wavelength = wavelength if wavelength.ndim == 1 else None
    ...
    y_test = y[test_idx]
    if ivar is not None:
        ivar_train = ivar[train_idx]
        ivar_val = ivar[val_idx]
        ivar_test = ivar[test_idx]
    else:
        ivar_train = ivar_val = ivar_test = None
    if observed_valid is not None:
        valid_train = observed_valid[train_idx]
        valid_val = observed_valid[val_idx]
        valid_test = observed_valid[test_idx]
    else:
        valid_train = valid_val = valid_test = None
    if shared_wavelength is None:
        wavelength_train = wavelength[train_idx]
        wavelength_val = wavelength[val_idx]
        wavelength_test = wavelength[test_idx]
    else:
        wavelength_train = shared_wavelength
        wavelength_val = shared_wavelength
        wavelength_test = shared_wavelength

    del X, y, all_indices, train_pool_idx, train_idx, val_idx, test_idx
    if ivar is not None:
        del ivar
    if observed_valid is not None:
        del observed_valid
    ...
    if args.use_ivar_channel:
        ...
        ivar_train_t = ivar_val_t = ivar_test_t = None
    if args.use_validity_channel:
        ...
        valid_train_t = valid_val_t = valid_test_t = None
    def wavelength_tensor(wavelength_array: np.ndarray, rows: int) -> torch.Tensor:
        tensor = torch.from_numpy(wavelength_array).float()
        if tensor.ndim == 1:
            tensor = tensor.unsqueeze(0).expand(rows, -1)
        return tensor

    wavelength_train_t = wavelength_tensor(wavelength_train, len(X_train))
    wavelength_val_t = wavelength_tensor(wavelength_val, len(X_val))
    wavelength_test_t = wavelength_tensor(wavelength_test, len(X_test))
    ...
    def build_input_tensor(
        ...
        ivar_tensor: torch.Tensor | None,
        valid_tensor: torch.Tensor | None,
    ) -> torch.Tensor:
        channels = [flux_tensor]
        if args.use_ivar_channel:
            if ivar_tensor is None:
                raise RuntimeError("IVAR channel requested but IVAR tensor is missing")
            channels.append(ivar_tensor)
        if args.use_validity_channel:
            if valid_tensor is None:
                raise RuntimeError("Validity channel requested but validity tensor is missing")
    ...
    if args.resume is not None and args.init_model is not None:
        raise ValueError("Use either --resume or --init-model, not both.")
    if args.init_model is not None:
        logger.info("Initializing model weights from: %s", args.init_model)
        init_state = torch.load(args.init_model, map_location=device, weights_only=True)
        ...
        *,
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.save_epoch_checkpoints:
                ...
                logger.info("Saved latest checkpoint to: %s", latest_checkpoint_path)
    ...
    logger.info("Loaded records: %d", n_records)
