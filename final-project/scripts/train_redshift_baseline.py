...
def split_arrays(
    *arrays: np.ndarray,
    ...
    if any(len(array) != n_items for array in arrays):
    ...
    n_test = int(np.ceil(n_items * test_size)) if test_size < 1.0 else int(test_size)
    n_test = min(max(n_test, 1), n_items - 1)
    test_idx = indices[:n_test]
    train_idx = indices[n_test:]
    split: list[np.ndarray] = []
    for array in arrays:
        split.extend([array[train_idx], array[test_idx]])
    return tuple(split)


def is_materialized_shard_payload(payload: dict[str, object]) -> bool:
    return {"flux", "ivar", "valid", "wavelength", "z"}.issubset(payload)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="For materialized shard inputs, shuffle shard order before applying --max-items.",
    )
    parser.add_argument(
        "--resume",
        type=Path,
...
def main() -> None:
    ...
    from data_pipeline import collate_records, load_hdf5, load_materialized_shards_batch, materialized_shard_to_batch
    ...
    if args.input.suffix == ".pt":
        ...
        logger.info("Loading cached processed bundle or materialized shard: %s", args.input)
        bundle = torch.load(args.input, map_location="cpu", weights_only=True)
        batch = materialized_shard_to_batch(bundle) if is_materialized_shard_payload(bundle) else bundle.get("batch", bundle)
        batch = (
            load_materialized_shards_batch(
                args.input,
                max_items=args.max_items,
                shard_shuffle_seed=args.subset_shuffle_seed,
            )
            if is_materialized_shard_payload(bundle)
            else batch
        )
        X = batch["flux"].numpy()
    ...
    elif args.input.is_dir() or args.input.name == "shards_manifest.jsonl":
        logger.info("Loading materialized shards: %s", args.input)
        batch = load_materialized_shards_batch(
            args.input,
            max_items=args.max_items,
            shard_shuffle_seed=args.subset_shuffle_seed,
        )
